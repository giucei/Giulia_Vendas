...
def get_produtos(search: str = "", categoria: str = "", sort: str = ""):
    ...
    cursor = conn.cursor()
    query = "SELECT id, nome, descricao, preco, estoque, categoria, sku, imagem FROM produtos WHERE 1=1"
    params = []
    if search:
        query += " AND nome LIKE ?"
    ...
    conn.close()
    return [Produto(id=row[0], nome=row[1], descricao=row[2], preco=row[3], estoque=row[4], categoria=row[5], sku=row[6], imagem=row[7]) for row in produtos]
...
def create_produto(produto: Produto):
    ...
    try:
        cursor.execute("INSERT INTO produtos (nome, descricao, preco, estoque, categoria, sku, imagem) VALUES (?, ?, ?, ?, ?, ?, ?)",
                       (produto.nome, produto.descricao, produto.preco, produto.estoque, produto.categoria, produto.sku, produto.imagem))
        conn.commit()
...
def update_produto(produto_id: int, produto: Produto):
    ...
    try:
        cursor.execute("UPDATE produtos SET nome=?, descricao=?, preco=?, estoque=?, categoria=?, sku=?, imagem=? WHERE id=?",
                       (produto.nome, produto.descricao, produto.preco, produto.estoque, produto.categoria, produto.sku, produto.imagem, produto_id))
        conn.commit()
    except Exception as e:
        conn.rollback()
